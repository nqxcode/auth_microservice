...
//go:generate minimock -i github.com/nqxcode/auth_microservice/internal/service.CacheUserService -o ./mocks\cache_user_service_minimock.go -n CacheUserServiceMock
...
func (m *CacheUserServiceMock) MinimockSetListDone() bool {
	...
	// if default expectation was set then invocations count should be greater than zero
	if m.SetListMock.defaultExpectation != nil && mm_atomic.LoadUint64(&m.afterSetListCounter) < 1 {
		return false
	}
	// if func was set then invocations count should be greater than zero
	if m.funcSetList != nil && mm_atomic.LoadUint64(&m.afterSetListCounter) < 1 {
		return false
	}
	return true
}
...
func (m *CacheUserServiceMock) MinimockSetPartialDone() bool {
	...
	// if default expectation was set then invocations count should be greater than zero
	if m.SetPartialMock.defaultExpectation != nil && mm_atomic.LoadUint64(&m.afterSetPartialCounter) < 1 {
		return false
	}
	// if func was set then invocations count should be greater than zero
	if m.funcSetPartial != nil && mm_atomic.LoadUint64(&m.afterSetPartialCounter) < 1 {
		return false
	}
	return true
}
